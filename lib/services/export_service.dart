...
class ExportService {
  static List<SpectrumPoint> _byType(SpectrumResult r, SpectrumType t) {
    switch (t) {
      case SpectrumType.elastic:
        return r.elasticSpectrum;
      case SpectrumType.epu:
        return r.epuSpectrum;
      case SpectrumType.design:
        return r.designSpectrum;
    }
  }
  ...
  static String generateSap2000Txt(
    SpectrumResult result, {
    String projectName = 'Proyecto Estructural',
    SpectrumType type = SpectrumType.design,
  }) {
    final sb = StringBuffer();
    final now = DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now());
    final pts = _byType(result, type);
    sb.writeln(
      r'$ =================================================================',
    );
    ...
    sb.writeln(r'$ Software: SASID Flutter Pro v0.2.0');
    sb.writeln(
      r'$ Tr: '
      '${result.factors.returnPeriod.label} | Época: ${result.factors.epoch.label} (${result.site.activeMallaHex})',
    );
    sb.writeln(
      r'$ -----------------------------------------------------------------',
    );
    ...
    sb.writeln(
      r'$ Latitud: '
      '${result.site.latitude.toStringAsFixed(6)}',
    );
    sb.writeln(
      r'$ Longitud: '
      '${result.site.longitude.toStringAsFixed(6)}',
    );
    sb.writeln(
      r'$ Zona Geotécnica: '
    ...
    sb.writeln(
      r'$ Periodo del Suelo (Ts): '
      '${result.site.ts.toStringAsFixed(3)} s',
    );
    sb.writeln(
      r'$ Aceleración del Terreno (a0): '
      '${result.site.a0.toStringAsFixed(3)} g',
    );
    sb.writeln(
      r'$ Coeficiente Sísmico (c): '
      '${result.site.c.toStringAsFixed(3)}',
    );
    sb.writeln(
      r'$ Periodos Meseta (Ta - Tb): '
      '${result.site.ta.toStringAsFixed(3)} s - ${result.site.tb.toStringAsFixed(3)} s',
    );
    sb.writeln(
      r'$ Exponente k: '
      '${result.site.k.toStringAsFixed(3)}',
    );
    sb.writeln(
      r'$ -----------------------------------------------------------------',
    );
    ...
    sb.writeln(
      r'$ Grupo de Importancia: '
      '${result.factors.importanceGroup.name} (I = ${result.factors.importanceGroup.factor})',
    );
    sb.writeln(
      r'$ Factor Comportamiento Sísmico (Q): '
      '${result.factors.q}',
    );
    sb.writeln(
      r'$ Factor Irregularidad (alpha): '
      '${result.factors.irregularity.factor}',
    );
    sb.writeln(
      r'$ Factor Hiperestaticidad (k1): '
      '${result.factors.k1}',
    );
    sb.writeln(
      r'$ Factor Sobrerresistencia Base (R0): '
      '${result.r0}',
    );
    sb.writeln(
      r'$ Aceleración Máxima de Diseño (aMax): '
    ...
    for (final pt in pts) {
      sb.writeln(
        '${pt.period.toStringAsFixed(4)}\t${pt.acceleration.toStringAsFixed(6)}',
      );
    }
    return sb.toString();
  }
  ...
  static String generateCsv(
    ...
    sb.writeln(
      '"Grupo de Importancia","${result.factors.importanceGroup.code} (I=${result.factors.importanceGroup.factor})",""',
    );
    sb.writeln('"Factor Q","${result.factors.q}",""');
    sb.writeln(
      '"Factor Irregularidad","${result.factors.irregularity.factor}",""',
    );
    sb.writeln('"Factor k1","${result.factors.k1}",""');
    sb.writeln('"Factor R0","${result.r0}",""');
    sb.writeln('"Tr","${result.factors.returnPeriod.label}",""');
    sb.writeln(
      '"Época","${result.factors.epoch.label} (${result.site.activeMallaHex})",""',
    );
    sb.writeln('"Aceleración Máxima de Diseño","${result.aMaxDesign}","g"');
    sb.writeln('');
    sb.writeln(
      '"Periodo T (s)","Espectro Diseño Sa (g)","Espectro Elástico Sa (g)","EPU Sa (g)"',
    );
    for (int i = 0; i < result.designSpectrum.length; i++) {
      ...
      sb.writeln(
        '${t.toStringAsFixed(3)},${sd.toStringAsFixed(6)},${se.toStringAsFixed(6)},${epu.toStringAsFixed(6)}',
      );
    }
    return sb.toString();
  }
  ...
  static Future<void> exportPdfReport(
    SpectrumResult result, {
    String projectName = 'Memoria de Cálculo Sísmico',
  }) async {
    ...
    for (final pt in result.designSpectrum) {
      if ((pt.period * 10).round() % 1 == 0 ||
          (pt.period == result.site.ta) ||
          (pt.period == result.site.tb)) {
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
                ...
                child: pw.Container(
                  ...
                  child: pw.Column(
                    ...
                    children: [
                      ...
                      _pdfRow(
                        'Época/Malla:',
                        '${result.factors.epoch.label} (${result.site.activeMallaHex})',
                      ),
                      _pdfRow('Tr:', result.factors.returnPeriod.label),
                    ],
                  ),
                ),
              ),
                ...
                child: pw.Container(
                  ...
                  child: pw.Column(
                    ...
                    children: [
                      ...
                      _pdfRow(
                        'Importancia:',
                        '${result.factors.importanceGroup.code} (I = ${result.factors.importanceGroup.factor})',
                      ),
                      _pdfRow('Factor Q:', result.factors.q.toString()),
                      _pdfRow(
                        'Irregularidad (α):',
                        result.factors.irregularity.factor.toString(),
                      ),
                      _pdfRow(
                        'Hiperestaticidad (k1):',
                        result.factors.k1.toString(),
                      ),
                      ...
                      pw.Container(
                        ...
                        child: pw.Text(
                          'Cálculo conforme al Apéndice A de las NTC-Sismo. Tr=${result.factors.returnPeriod.label}',
                          style: pw.TextStyle(
                            fontSize: 8,
          ...
          pw.SizedBox(height: 14),
          pw.Text(
            'TABLA DE ORDENADAS ESPECTRALES',
            style: pw.TextStyle(
              fontWeight: pw.FontWeight.bold,
              fontSize: 10,
              color: PdfColor.fromHex('1E3A8A'),
